...
import { Subscription, SubscriptionStatus, PaymentStatus } from '../models/Subscription';
...
import { In, MoreThanOrEqual, LessThanOrEqual, ILike, Repository } from 'typeorm';
...
export class SubscriptionService {
    ...
    private async findPlanByIdentifier(planRepository: Repository<SubscriptionPlan>, identifier: string): Promise<SubscriptionPlan | null> {
        if (!identifier) {
            return null;
        }
     ...
     */
    async createSubscription(data: CreateSubscriptionData): Promise<{ subscription: Subscription; payment: Payment }> {
     ...
     */
    async processPayment(paymentId: string, gatewayTransactionId: string, success: boolean, failureReason?: string): Promise<Payment> {
        return await AppDataSource.transaction(async manager => {
            const paymentRepository = manager.getRepository(Payment);
            const subscriptionRepository = manager.getRepository(Subscription);
    ...
    async getUserActiveSubscription(userId: string): Promise<Subscription | null> {
        return await this.subscriptionRepository
            ...
            .andWhere('subscription.status IN (:...statuses)', { statuses: ['active', 'trial'] })
    ...
    async finalizeCashfreeReturn(params: {
        ...
        if (!resolvedOrderId && paymentSessionId) {
            ...
            if (seedPayment) {
                resolvedOrderId = (seedPayment.metadata?.cashfree?.orderId as string | undefined)?.trim() || seedPayment.transactionId;
            }
        }
        ...
        try {
            type InterimResult =
                ...
                | { outcome: 'SUCCESS'; paymentId: string; gatewayTransactionId: string; status: string }
                | { outcome: 'FAILED'; paymentId: string; gatewayTransactionId: string; status: string }
                | { outcome: 'PENDING'; payment: Payment; subscription: Subscription | null; status: string };
            ...
            const interim = await AppDataSource.transaction(async manager => {
                ...
                if (!payment) {
                    return { outcome: 'NOT_FOUND', status: hintedStatus || 'UNKNOWN', message: 'Payment not found' } as InterimResult;
                }
                ...
                if (verifyWithGateway && (!effectiveStatus || effectiveStatus === 'PENDING')) {
                    try {
                        const gatewayOrder = await cashfreePaymentService.getOrder(resolvedOrderId);
            ...
            logger.error('Failed to finalize Cashfree return', error, { orderId: resolvedOrderId });
            return {
                status: 'ERROR',
     ...
     * Renew subscription
     */
    async renewSubscription(subscriptionId: string, paymentMethod?: PaymentMethod): Promise<{ subscription: Subscription; payment: Payment | null }> {
        return await AppDataSource.transaction(async manager => {
            const subscriptionRepository = manager.getRepository(Subscription);
            const paymentRepository = manager.getRepository(Payment);
     ...
     */
    async checkSubscriptionLimits(userId: string, resource: keyof SubscriptionUsage, currentUsage: number): Promise<{ allowed: boolean; limit: number; usage: number; remaining: number }> {
     ...
     */
    async handleWebhook(gatewayTransactionId: string, status: string, metadata: Record<string, any> = {}): Promise<void> {
     ...
     */
    async updateUsageTracking(subscriptionId: string, usage: Partial<SubscriptionUsage>): Promise<void> {
        const subscription = await this.subscriptionRepository.findOne({
            where: { id: subscriptionId }
        });
