...
export const signup = async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
        ...
        if (existingUser) {
            ...
            if (!existingUser.isVerified) {
                ...
                if (canRegenerateOTP) {
                    ...
                    // Generate new OTP
                    const otp = await otpService.generateAndSaveOTP(existingUser, 'verification');
                    ...
                    if (!emailSent) {
                        logger.warn('Failed to send OTP email, but user was updated', { email });
                    }
        ...
        const otp = await otpService.generateAndSaveOTP(newUser, 'verification');
        ...
        if (!emailSent) {
            logger.warn('Failed to send OTP email, but user was created', { email });
        }
...
export const verifyOtp = async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
        ...
        if (!otp || !otpService.validateOTPFormat(otp)) {
            return res.status(400).json({ 
                success: false,
        ...
        const otpValidation = otpService.validateOTP(user, otp);
...
export const forgotPassword = async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
        ...
        if (!otpService.canRequestNewOTP(user)) {
        ...
        // Generate and save OTP using the service
        const otp = await otpService.generateAndSaveOTP(user, 'password_reset');
        
        // Send OTP email
        const emailSent = await emailService.sendOtpEmail(email, otp, 'password_reset');
        
        if (!emailSent) {
            logger.warn('Failed to send password reset OTP email', { email });
        }
...
export const resendOtp = async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
        ...
        const otp = await otpService.generateAndSaveOTP(user, purpose as 'verification' | 'password_reset');
        
        // Send OTP email
        const emailSent = await emailService.sendOtpEmail(email, otp, purpose as 'verification' | 'password_reset');
        
        if (!emailSent) {
            logger.warn('Failed to send OTP email', { email });
        }
...
export const getOtpConfig = async (req: Request, res: Response): Promise<Response> => {
    try {
