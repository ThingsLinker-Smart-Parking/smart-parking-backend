...
exports.handleChirpStackWebhook = exports.getDashboardStats = exports.updateNodeData = exports.getSlotDetails = exports.getParkingOverview = void 0;
 ...
 */
const getParkingOverview = async (req, res) => {
    try {
        ...
        const result = Array.from(parkingData.values()).map(lot => {
            const floors = Array.from(lot.floors.values()).map((floor) => ({
                id: floor.id,
                name: floor.name,
                level: floor.level,
                slots: floor.slots,
                totalSlots: floor.slots.length,
                availableSlots: floor.slots.filter((s) => s.status === 'available').length,
                occupiedSlots: floor.slots.filter((s) => s.status === 'occupied').length,
                unknownSlots: floor.slots.filter((s) => s.status === 'unknown').length,
                unmonitoredSlots: floor.slots.filter((s) => s.status === 'unmonitored').length
            }));
            return {
                ...lot,
                floors,
        ...
        return apiResponse_1.ApiResponseBuilder.success({
            parkingLots: result,
            statistics: overallStats
        }, 'Parking overview retrieved successfully').send(res);
    }
    catch (error) {
        loggerService_1.logger.error('Get parking overview error:', error);
        return apiResponse_1.ApiResponseBuilder.error('Failed to retrieve parking overview').send(res, 500);
    }
};
exports.getParkingOverview = getParkingOverview;
/**
...
const getSlotDetails = async (req, res) => {
    ...
    try {
        const idValidation = (0, validation_1.validateUuidParam)(slotId, 'slotId');
        if (!idValidation.isValid) {
            return apiResponse_1.ApiResponseBuilder.error(idValidation.error || 'Invalid slot ID').send(res, 400);
        }
        const parkingSlotRepository = data_source_1.AppDataSource.getRepository(ParkingSlot_1.ParkingSlot);
        const nodeRepository = data_source_1.AppDataSource.getRepository(Node_1.Node);
        ...
        if (!parkingSlot) {
            return apiResponse_1.ApiResponseBuilder.notFound('Parking slot').send(res, 404);
        }
        // Get associated node
        ...
        return apiResponse_1.ApiResponseBuilder.success(result, 'Slot details retrieved successfully').send(res);
    }
    catch (error) {
        loggerService_1.logger.error('Get slot details error:', error);
        return apiResponse_1.ApiResponseBuilder.error('Failed to retrieve slot details').send(res, 500);
    }
};
exports.getSlotDetails = getSlotDetails;
/**
...
const updateNodeData = async (req, res) => {
    ...
    try {
        const idValidation = (0, validation_1.validateUuidParam)(nodeId, 'nodeId');
        if (!idValidation.isValid) {
            return apiResponse_1.ApiResponseBuilder.error(idValidation.error || 'Invalid node ID').send(res, 400);
        }
        const nodeRepository = data_source_1.AppDataSource.getRepository(Node_1.Node);
        const statusLogRepository = data_source_1.AppDataSource.getRepository(ParkingStatusLog_1.ParkingStatusLog);
        const node = await nodeRepository.findOne({
        ...
        if (!node) {
            return apiResponse_1.ApiResponseBuilder.notFound('Node').send(res, 404);
        }
        // Validate input
        if (percentage !== undefined && (percentage < 0 || percentage > 100)) {
            return apiResponse_1.ApiResponseBuilder.error('Percentage must be between 0 and 100').send(res, 400);
        }
        if (batteryLevel !== undefined && (batteryLevel < 0 || batteryLevel > 100)) {
            return apiResponse_1.ApiResponseBuilder.error('Battery level must be between 0 and 100').send(res, 400);
        }
        ...
        loggerService_1.logger.info('Node data updated manually', {
            nodeId: node.id,
            adminId: req.user.id,
            distance,
            percentage,
        ...
        loggerService_1.logger.error('Update node data error:', error);
        return apiResponse_1.ApiResponseBuilder.error('Failed to update node data').send(res, 500);
    }
};
    ...
    try {
        ...
        for (const slot of allSlots) {
            ...
            if (node) {
                ...
                else if (persistedStatus === 'occupied')
                    slotStatusCounts.occupied++;
                else if (persistedStatus === 'reserved')
        ...
        loggerService_1.logger.error('Get dashboard stats error:', error);
        return apiResponse_1.ApiResponseBuilder.error('Failed to retrieve dashboard statistics').send(res, 500);
    }
};
 ...
 */
const handleChirpStackWebhook = async (req, res) => {
    try {
        const chirpstackData = req.body;
        loggerService_1.logger.info('ChirpStack webhook data received', {
        ...
        loggerService_1.logger.error('ChirpStack webhook error:', error);
        return apiResponse_1.ApiResponseBuilder.error('Failed to process ChirpStack webhook').send(res, 500);
    }
};
exports.handleChirpStackWebhook = handleChirpStackWebhook;
